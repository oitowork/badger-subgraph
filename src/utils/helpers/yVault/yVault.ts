...
export function getOrCreateAffiliateVault(vaultAddress: Address, update: boolean = true): Vault {
  let vault = Vault.load(vaultAddress.toHexString());
  ...
  if (update) {
    ...
    let balance = vaultContract.try_totalVaultBalance(Address.fromString(vaultAddress.toHexString()));
